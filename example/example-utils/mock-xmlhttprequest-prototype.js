...
// HUGE thanks to https://github.com/driverdan/node-XMLHttpRequest
...
const MockXMLHttpRequest = function() {
    ...
    var isAllowedHttpHeader = function(header) {
        return disableHeaderCheck || 
           (header && forbiddenRequestHeaders
    ...
    this.send = function(data) {
        if (this.readyState !== this.OPENED) {
            throw new Error(
                "INVALID_STATE_ERR: connection must be opened before send() " + 
                "is called");
        }
        ...
        var responseHandler = function responseHandler(resp) {
            ...
            if (
                ...
                var newOptions = {
                    ...
                    port: url.port,
                    path: url.path,
                    method: response.statusCode === 303 
                            ? "GET" 
                            : settings.method,
                    headers: headers,
                    withCredentials: self.withCredentials
